...
const uniqueSlug = async (db: Connection, base: string): Promise<string> => {
  ...
  for (let i = 0; i < 50; i++) {
    const taken = await db.one(
      from("spaces")
        .where(q => q("slug").equals(candidate))
        .select("id"),
    )
      ...
      authed(async c => {
        ...
        if (body.name !== undefined) {
          if (typeof body.name !== "string" || !body.name.trim())
            return json(c, 422, { error: "name must be a non-empty string" })
        ...
        const fresh = (await db.one(from("spaces").where(q => q("id").equals(id)))) as SpaceRow
