...
import { requireAuth, hashToken } from "../auth/guard.ts"
...
import { sendSystem, broadcastSystem } from "../messages/system.ts"
...
export const adminUserRoutes = (db: Connection, secret: string, emailer: Emailer, appUrl: string) => {
  ...
  return [
    get("/admin/users/:id", guard(async (c) => {
      ...
      const user = await db.one(
        from("users")
          ...
          .select(
            "id", "username", "email", "name", "is_owner",
            "storage_quota_bytes", "totp_enabled",
    ...
    patch("/admin/users/:id", authed(async (c) => {
      ...
      if (body.username !== undefined) {
        ...
        if (!isValidUsername(username)) return json(c, 422, { error: "Username must be 3-32 chars, lowercase letters, digits, and underscores" })
        const taken = await db.one(
          from("users")
      ...
      await db.execute(from("users").where(q => q("id").equals(id)).update(update))
    ...
    post("/admin/users/:id/suspend", authed(async (c) => {
      ...
      void sendSystem(db, id, "Your account has been suspended",
        body.reason
    ...
    post("/admin/users/:id/unsuspend", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      const target = await db.one(
        from("users").where(q => q("id").equals(id)).select("id", "suspended_at"),
      ...
      void sendSystem(db, id, "Your account has been restored",
    ...
    post("/admin/users/:id/reset-password", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      const target = await db.one(
        from("users").where(q => q("id").equals(id)).select("id", "name", "email", "deleted_at"),
    ...
    post("/admin/users/:id/message", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
    ...
    post("/admin/broadcast", authed(async (c) => {
      const userId = authId(c)
      const body = c.body as { subject?: string; body?: string }
      const subject = body.subject?.trim()
      const messageBody = body.body?.trim()
