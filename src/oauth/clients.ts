...
export const oauthClientRoutes = (db: Connection, secret: string) => {
  ...
  return [
    get(
      "/admin/oauth/clients",
      guard(async c => {
        const rows = (await db.all(from("oauth_clients").orderBy("created_at", "DESC"))) as ClientRow[]
    ...
    patch(
      ...
      authed(async c => {
        ...
        const fresh = (await db.one(from("oauth_clients").where(q => q("id").equals(id)))) as ClientRow | null
