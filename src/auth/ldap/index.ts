...
export const ldapRoutes = (db: Connection, secret: string) => {
  ...
  return [
    ...
    post("/auth/ldap/login", api(async (c) => {
      ...
      if (!idRate.ok) return json(c, 429, { error: "Too many attempts for this account.", retry_after: idRate.retryAfterSeconds })
      ...
      try {
        ...
        logEvent(db, { event: "ldap.provision_fail", metadata: { identity, reason: (err as Error).message }, ip, userAgent: ua })
