...
const hashCodes = async (codes: string[]): Promise<string[]> => Promise.all(codes.map(c => hash(c)))
    ...
    post(
      ...
      guard(async c => {
        ...
        const user = (await db.one(
          from("users")
            .where(q => q("id").equals(userId))
            .select("email", "username", "totp_enabled"),
    ...
    post(
      ...
      authed(async c => {
        ...
        const user = (await db.one(
          from("users")
            .where(q => q("id").equals(userId))
            .select("password", "totp_secret", "totp_enabled"),
        )) as { password: string; totp_secret: string | null; totp_enabled: boolean } | null
        if (!user?.totp_enabled || !user.totp_secret) return json(c, 409, { error: "MFA not enabled" })
    ...
    post(
      ...
      authed(async c => {
        ...
        const user = (await db.one(
          from("users")
            .where(q => q("id").equals(userId))
            .select("password", "totp_enabled"),
        )) as { password: string; totp_enabled: boolean } | null
        if (!user?.totp_enabled) return json(c, 409, { error: "MFA not enabled" })
