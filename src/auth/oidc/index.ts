...
export const sweepExpiredOidcStates = async (db: Connection): Promise<void> => {
  try {
    await db.execute(
      from("oidc_states").where(q => q("expires_at").lessThan(raw("NOW()"))).del(),
    )
  } catch (err) {
...
export const oidcRoutes = (db: Connection, secret: string, appUrl: string) => {
  ...
  return [
    ...
    get("/auth/oidc/start", pre(async (c) => {
      const cfg = await loadOidcConfig(db)
      if (!isOidcReady(cfg)) {
    ...
    get("/auth/oidc/callback", pre(async (c) => {
      const url = new URL(c.request.url)
      ...
      await db.execute(from("oidc_states").where(q => q("state").equals(state)).del())
      ...
      if (!tokenRes.ok) {
        ...
        logEvent(db, { event: "oidc.token_exchange_failed", metadata: { status: tokenRes.status, body: body.slice(0, 500) } })
      ...
      let user
      try {
        const result = await upsertFromExternal(db, {
          provider: "oidc",
          subject: claims.sub,
      ...
      }, secret, { ip: clientIp(c.request), userAgent: userAgent(c.request) })
