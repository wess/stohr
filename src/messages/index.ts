...
const hydrate = (row: MessageRow) => ({
  ...
  from: row.from_user_id == null
    ? null
    : { id: row.from_user_id, username: row.from_username, name: row.from_name },
...
const resolveRecipient = async (
  ...
  if (id) {
    return await db.one(
      from("users").where(q => q("id").equals(id)).where(q => q("deleted_at").isNull()).select("id"),
    ) as { id: number } | null
  }
  const username = input.username
  if (username) {
    return await db.one(
      from("users").where(q => q("username").equals(username.toLowerCase())).where(q => q("deleted_at").isNull()).select("id"),
    ) as { id: number } | null
  }
  const email = input.email
  if (email) {
    return await db.one(
      from("users").where(q => q("email").equals(email.toLowerCase())).where(q => q("deleted_at").isNull()).select("id"),
    ) as { id: number } | null
  }
  return null
}
...
export const messageRoutes = (db: Connection, secret: string) => {
  ...
  return [
    ...
    get("/me/messages/unread-count", guard(async (c) => {
      const userId = authId(c)
    ...
    get("/me/messages/thread/:threadId", guard(async (c) => {
      const userId = authId(c)
    ...
    post("/me/messages", authed(async (c) => {
      const userId = authId(c)
      const body = c.body as {
        to_user_id?: number; toUserId?: number; username?: string; email?: string;
    ...
    post("/me/messages/:id/reply", authed(async (c) => {
      const userId = authId(c)
      ...
      const target = await db.one(
        from("messages").where(q => q("id").equals(id)).select(
          "id", "thread_id", "from_user_id", "to_user_id", "subject", "kind", "deleted_at",
        ),
      ) as { id: number; thread_id: number; from_user_id: number | null; to_user_id: number; subject: string; kind: string; deleted_at: string | null } | null
    ...
    post("/me/messages/:id/read", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      await db.execute(
        from("messages")
    ...
    post("/me/messages/read-all", authed(async (c) => {
      const userId = authId(c)
      await db.execute(
        from("messages")
          .where(q => q("to_user_id").equals(userId))
    ...
    post("/me/messages/:id/archive", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      await db.execute(
        from("messages")
    ...
    post("/me/messages/:id/unarchive", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      await db.execute(
        from("messages")
    ...
    del("/me/messages/:id", guard(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
