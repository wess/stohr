  ...
  return [
    ...
    post(
      ...
      upload(async c => {
        ...
        if (!body?.files) return json(c, 422, { error: "No file uploaded" })
        ...
        // Initial scan verdict: 'pending' when clamd is configured (the sweep
        // will scan it), 'skipped' otherwise so downloads are never gated.
        ...
        for (const file of entries) {
          ...
          if (existing) {
            ...
            fileId = rows[0]!.id
            isNewVersion = false
            dispatchWebhook(db, ownerId, "file.created", {
              event: "file.created",
              file_id: fileId,
              name,
              mime,
          ...
          if (targetFolder) {
            const fresh = (await db.one(from("files").where(q => q("id").equals(fileId)))) as FileRow | null
            if (fresh) {
    ...
    patch(
      ...
      guard(async c => {
        ...
        const updatedFile = (await db.one(from("files").where(p => p("id").equals(id)))) as FileRow | null
          ...
          if (moved) {
            if (oldFolderId != null) {
              const oldFolder = (await db.one(
                from("folders")
                  .where(q => q("id").equals(oldFolderId))
                  .where(q => q("deleted_at").isNull()),
              )) as FolderRow | null
              if (oldFolder) {
                summaries.push(
                  ...(await fireEvent({
                    db,
                    store,
                    event: "file.moved.out",
                    folder: oldFolder,
                    subject: { kind: "file", row: updatedFile },
                    actor: { id: userId },
                  })),
                )
              }
            ...
            const folder = (await db.one(
              from("folders")
                .where(q => q("id").equals(updatedFile.folder_id))
                .where(q => q("deleted_at").isNull()),
            )) as FolderRow | null
            if (folder) {
              summaries.push(
    ...
    del(
      ...
      guard(async c => {
        ...
        dispatchWebhook(db, file.user_id, "file.deleted", {
          event: "file.deleted",
          file_id: file.id,
          name: file.name,
          mime: file.mime,
