...
const REGISTRY = {
  [SETTING_WEBDAV_ENABLED]: {
    default: false,
    type: "boolean" as const,
    description:
      "WebDAV endpoint at /webdav. Users still need to mint a per-account WebDAV password before they can connect.",
  },
  [SETTING_FEDERATION_ENABLED]: {
    default: false,
    type: "boolean" as const,
    description:
      "Federation features (mint, join, mount). Existing federations remain in the DB when this is off — they just stop accepting traffic until re-enabled.",
  },
  [SETTING_MCP_ENABLED]: {
    default: false,
    type: "boolean" as const,
    description:
      "Model Context Protocol server at /mcp. When off the endpoint returns 503 regardless of per-tool toggles. AI clients (Claude Desktop, IDEs) authenticate with a PAT or OAuth access token.",
  },
  [SETTING_MCP_TOOL_READ]: {
    default: true,
    type: "boolean" as const,
    description:
      "Expose read-only MCP tools: list folders/files, read file contents, search. Safe default — these only return data the caller's token already has access to.",
  },
  [SETTING_MCP_TOOL_WRITE]: {
    default: false,
    type: "boolean" as const,
    description:
      "Expose write MCP tools: create folder, upload file, rename, move. Off by default — opt in once you trust the AI client and its prompts.",
  },
  [SETTING_MCP_TOOL_DELETE]: {
    default: false,
    type: "boolean" as const,
    description:
      "Expose destructive MCP tools: trash, restore, purge. Soft-deletes go to /trash and can be restored, but purge is permanent.",
  },
  [SETTING_MCP_TOOL_SHARE]: {
    default: false,
    type: "boolean" as const,
    description:
      "Expose sharing MCP tools: create and revoke public share links. Anything the AI shares becomes reachable without auth — keep off unless you specifically want this.",
  },
}
...
export const getBoolean = async (db: Connection, key: SettingKey): Promise<boolean> => {
  ...
  try {
    ...
    return REGISTRY[key].default as boolean
  }
}
...
export const requireSettingEnabled =
  (db: Connection, key: SettingKey) =>
  async (conn: Conn): Promise<Conn> => {
    const enabled = await getBoolean(db, key)
    if (!enabled) {
      return halt(conn, 503, {
        error: `${key} is disabled on this instance. Ask the owner to enable it in Admin → Settings.`,
      })
    }
    return conn
  }
...
export const requireSettingEnabledBasic =
  (db: Connection, key: SettingKey) =>
  async (conn: Conn): Promise<Conn> => {
    const enabled = await getBoolean(db, key)
    if (!enabled) {
