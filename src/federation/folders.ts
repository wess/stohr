...
type EnsureMemberOk = { ok: true; fed: NonNullable<Awaited<ReturnType<typeof federationById>>>; member: NonNullable<Awaited<ReturnType<typeof localMemberFor>>> }
type EnsureMemberErr = { ok: false; status: number; body: { error: string } }

const ensureMember = async (db: Connection, federationId: number, userId: number): Promise<EnsureMemberOk | EnsureMemberErr> => {
...
export const federationFolderRoutes = (db: Connection, secret: string) => {
  ...
  return [
    ...
    post("/me/federations/:id/folders/:folder_id/contribute", authed(async (c) => {
      ...
      if (existing && existing.id !== folderId) {
        return json(c, 409, { error: "Already have a contribution folder for this federation", folder_id: existing.id })
      }

      await db.execute(
        from("folders").where(q => q("id").equals(folderId)).update({
          federation_id: id,
      ...
      await db.execute(
        from("federation_members").where(q => q("id").equals(ctx.member.id)).update({
    ...
    patch("/me/federations/:id/folders/:folder_id/contribute", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      const folderId = Number(c.params.folder_id)
      const body = c.body as ContributeBody
      ...
      await db.execute(
        from("folders").where(q => q("id").equals(folderId)).update({ federation_quota_bytes: quotaBytes }),
      )
      await db.execute(
        from("federation_members").where(q => q("id").equals(ctx.member.id)).update({ contributed_bytes: quotaBytes }),
      )
      return json(c, 200, { folder_id: folderId, quota_bytes: quotaBytes })
    })),

    del("/me/federations/:id/folders/:folder_id/contribute", guard(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      const folderId = Number(c.params.folder_id)
      ...
      await db.execute(
        from("federation_members").where(q => q("id").equals(ctx.member.id)).update({ contributed_bytes: 0 }),
      )
    ...
    post("/me/federations/:id/folders/mount", authed(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
    ...
    get("/me/federations/:id/usage", guard(async (c) => {
      const userId = authId(c)
      const id = Number(c.params.id)
      const ctx = await ensureMember(db, id, userId)
      if (!ctx.ok) return json(c, ctx.status, ctx.body)
