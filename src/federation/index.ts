...
export const federationRoutes = (db: Connection, secret: string, publicBaseUrl: string) => {
  ...
  return [
    ...
    post(
      ...
      authed(async c => {
        ...
        if (!slug || !slugRegex.test(slug))
          return json(c, 422, { error: "Invalid slug (lowercase alphanumeric + hyphens, 3-64 chars)" })
        ...
        if (replicationFactor < 1 || replicationFactor > 16)
          return json(c, 422, { error: "replication_factor must be 1-16" })
    ...
    del(
      "/me/federations/:id/invites/:invite_id",
      guard(async c => {
        const userId = authId(c)
        const id = Number(c.params.id)
    ...
    post(
      ...
      authed(async c => {
        ...
        logEvent(db, {
          ...
          metadata: { federation_id: fedId, slug: pairResponse.federation.slug },
        })
