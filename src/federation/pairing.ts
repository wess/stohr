...
import { fedPublicKeyRaw, federationById, federationBySlug, remoteMembersForFederation } from "./membership.ts"
import { mintInvite, markInviteUsed, parseInvite } from "./invites.ts"
...
export const pairingReceiverRoutes = (db: Connection, publicBaseUrl: string) => {
  ...
  return [
    post("/federation/pair", open(async (c) => {
      ...
      if (existing) {
        await db.execute(
          from("federation_members").where(q => q("id").equals(existing.id)).update({
...
export { peerFetch, parseInvite, mintInvite, aesGcmDecrypt, federationBySlug }
