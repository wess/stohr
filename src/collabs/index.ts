...
const accessFor = async (db: Connection, userId: number, kind: ResourceKind, resourceId: number) =>
  kind === "folder" ? await folderAccess(db, userId, resourceId) : await fileAccess(db, userId, resourceId)
...
const addCollab = (db: Connection, kind: ResourceKind, emailer: Emailer, appUrl: string) => async (c: any) => {
  ...
  if (target.kind === "user") {
    ...
    dispatchWebhook(db, userId, "collaboration.invited", {
      event: "collaboration.invited",
      collaboration_id: inserted[0].id,
      resource_type: kind,
      resource_id: id,
      invited_user_id: target.user.id,
      invited_username: target.user.username,
      invited_email: target.user.email,
      role,
      invited_by: userId,
    })
