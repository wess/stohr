...
import { remoteInitialize, remoteListTools, type RemoteServer } from "./client.ts"
...
export const loadEnabledServers = async (db: Connection): Promise<RemoteServer[]> => {
  const rows = await db.all(
    from("mcp_servers").where(q => q("enabled").equals(true)).select("url", "auth_token"),
...
export const mcpServerRoutes = (db: Connection, secret: string) => {
  ...
  return [
    get("/admin/mcp/servers", guard(async (c) => {
      const rows = await db.all(
        from("mcp_servers").orderBy("created_at", "DESC"),
      ) as ServerRow[]
      return json(c, 200, rows.map(redact))
    })),

    post("/admin/mcp/servers", authed(async (c) => {
      const userId = authId(c)
      const body = c.body as { name?: string; description?: string; url?: string; auth_token?: string; authToken?: string; enabled?: boolean }
      ...
      const rows = await db.execute(
        from("mcp_servers").insert({
          name, description, url, auth_token: authToken,
          enabled, created_by: userId,
        }).returning("id", "name", "description", "url", "auth_token", "enabled", "created_by", "created_at", "updated_at"),
      ) as ServerRow[]
      return json(c, 201, redact(rows[0]!))
    })),

    patch("/admin/mcp/servers/:id", authed(async (c) => {
      const id = Number(c.params.id)
      const body = c.body as { name?: string; description?: string | null; url?: string; auth_token?: string | null; authToken?: string | null; enabled?: boolean }
      ...
      await db.execute(from("mcp_servers").where(q => q("id").equals(id)).update(update))
      const fresh = await db.one(from("mcp_servers").where(q => q("id").equals(id))) as ServerRow
      return json(c, 200, redact(fresh))
    })),

    del("/admin/mcp/servers/:id", guard(async (c) => {
      const id = Number(c.params.id)
      const row = await db.one(from("mcp_servers").where(q => q("id").equals(id))) as ServerRow | null
      if (!row) return json(c, 404, { error: "Server not found" })
      await db.execute(from("mcp_servers").where(q => q("id").equals(id)).del())
    ...
    post("/admin/mcp/servers/:id/probe", guard(async (c) => {
      const id = Number(c.params.id)
      const row = await db.one(from("mcp_servers").where(q => q("id").equals(id))) as ServerRow | null
      if (!row) return json(c, 404, { error: "Server not found" })
