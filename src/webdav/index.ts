...
import { del, get, halt, json, parseJson, pipeline, post, putHeader, setStatus, stream, text } from "@atlas/server"
...
import { requireSettingEnabled, SETTING_WEBDAV_ENABLED, webdavEnabled } from "../settings/index.ts"
import { authenticateWebdav, generateWebdavToken, hashWebdavToken } from "./auth.ts"
import { decodeSegments, listChildren, resolvePath } from "./paths.ts"
import { renderMultistatus } from "./xml.ts"
...
const requireBasic = (db: Connection) => async (conn: Conn): Promise<Conn> => {
  const auth = await authenticateWebdav(db, conn.request.headers.get("authorization"))
  if (!auth) {
    return halt(
      putHeader(conn, "www-authenticate", 'Basic realm="Stohr WebDAV"'),
      401,
      { error: "Unauthorized" },
    )
  }
...
const webdavUser = (c: Conn): { userId: number; username: string } => (c.assigns as any).webdav_user
...
export const webdavRoutes = (db: Connection, store: StorageHandle, secret: string): Route[] => {
  const gate = requireSettingEnabled(db, SETTING_WEBDAV_ENABLED)
  const settings = pipeline(gate, requireAuth({ secret, db, noOAuth: true }))
  const settingsBody = pipeline(gate, requireAuth({ secret, db, noOAuth: true }), parseJson)
  const dav = pipeline(gate, requireBasic(db))

  return [
    // ────────────────────────────────────────────────────────────────────
    // Per-user WebDAV settings (regular auth)
    // ────────────────────────────────────────────────────────────────────
    get("/me/webdav", settings(async (c) => {
      const userId = authId(c)
      const row = await db.one(
        from("webdav_credentials").where(q => q("user_id").equals(userId)).select("enabled", "last_used_at", "updated_at"),
      ) as { enabled: boolean; last_used_at: string | null; updated_at: string } | null
      return json(c, 200, row ?? { enabled: false, last_used_at: null, updated_at: null })
    })),

    // Enable WebDAV and rotate the password. Returns the password ONCE —
    // it's hashed before storage and unrecoverable.
    post("/me/webdav", settingsBody(async (c) => {
      const userId = authId(c)
      const token = generateWebdavToken()
      const hash = hashWebdavToken(token)
      await db.execute({
        text: `INSERT INTO webdav_credentials (user_id, token_hash, enabled, updated_at)
               VALUES ($1, $2, TRUE, NOW())
               ON CONFLICT (user_id) DO UPDATE
                 SET token_hash = EXCLUDED.token_hash, enabled = TRUE, updated_at = NOW()`,
        values: [userId, hash],
      })
      return json(c, 201, { enabled: true, password: token })
    })),

    del("/me/webdav", settings(async (c) => {
      const userId = authId(c)
      await db.execute(
        from("webdav_credentials").where(q => q("user_id").equals(userId)).update({ enabled: false, token_hash: null, updated_at: raw("NOW()") }),
      )
      return json(c, 200, { disabled: true })
    })),

    // ────────────────────────────────────────────────────────────────────
    // WebDAV protocol routes (HTTP Basic auth)
    // ────────────────────────────────────────────────────────────────────
    ...
    webdavRoute("PROPFIND", "/webdav/*", dav(async (c) => {
      ...
      if (resolved.file) {
        ...
        const folderRow = resolved.folderId
          ? await db.one(from("folders").where(q => q("id").equals(resolved.folderId)).select("name", "created_at")) as { name: string; created_at: string } | null
          : null
        entries.push({
    ...
    webdavRoute("GET", "/webdav/*", dav(async (c) => {
      const u = webdavUser(c)
      const segments = pathFromConn(c)
      const resolved = await resolvePath(db, u.userId, segments)
      ...
      if (isFederationKey(f.storage_key)) {
        ...
        const headered = putHeader(
          putHeader(c, "content-type", f.mime),
          "content-length",
          String(bytes.length),
        )
    ...
    webdavRoute("HEAD", "/webdav/*", dav(async (c) => {
      const u = webdavUser(c)
      const segments = pathFromConn(c)
      const resolved = await resolvePath(db, u.userId, segments)
    ...
    webdavRoute("PUT", "/webdav/*", dav(async (c) => {
      const u = webdavUser(c)
      const segments = pathFromConn(c)
      ...
      if (folderId != null) {
        const parent = await db.one(
          from("folders").where(q => q("id").equals(folderId)).select("federation_role"),
      ...
      if (resolved.file) {
        ...
        await db.execute(
          from("files").where(q => q("id").equals(resolved.file!.fileId)).update({
            mime,
            size: bytes.length,
            storage_key: key,
          }),
    ...
    webdavRoute("DELETE", "/webdav/*", dav(async (c) => {
      const u = webdavUser(c)
      const segments = pathFromConn(c)
      ...
      if (resolved.file) {
        const f = resolved.file
        await db.execute(from("files").where(q => q("id").equals(f.fileId)).del())
        if (isFederationKey(f.storage_key)) {
    ...
    webdavRoute("MOVE", "/webdav/*", dav(async (c) => {
      const u = webdavUser(c)
      const segments = pathFromConn(c)
      ...
      if (src.file) {
        if (dst.exists && dst.file) {
          ...
          await db.execute(from("files").where(q => q("id").equals(dst.file!.fileId)).del())
          if (isFederationKey(dst.file.storage_key)) await dropFederationBlob(db, store, dst.file.storage_key).catch(() => {})
        ...
        await db.execute(
          from("files").where(q => q("id").equals(src.file!.fileId)).update({ name: newName, folder_id: newFolderId }),
        )
      ...
      await db.execute(
        from("folders").where(q => q("id").equals(src.folderId!)).update({ name: newName, parent_id: newParentId }),
      )
      return setStatus(c, 201)
    })),

    webdavRoute("COPY", "/webdav/*", dav(async (c) => {
      const u = webdavUser(c)
      const segments = pathFromConn(c)
      const destinationHeader = c.request.headers.get("destination")
      if (!destinationHeader) return halt(c, 400, { error: "Missing Destination header" })
      let destPath: string
      try { destPath = new URL(destinationHeader).pathname } catch { destPath = destinationHeader }
      if (!destPath.startsWith(WEBDAV_PREFIX)) return halt(c, 400, { error: "Destination outside WebDAV root" })
      const destSegments = decodeSegments(destPath.slice(WEBDAV_PREFIX.length))
